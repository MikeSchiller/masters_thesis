...
class i2c_hmc5883l:
	...
	def getAxes(self):
		...
		#Vom RASPI DER EINDEUTIG WEI?; DASS DAS NOCH EXISTIERT
		import math
from i2clibraries import i2c
from time import *
...
class i2c_hmc5883l:
	...
	def getTilt(self):
		...
		headingRad = math.atan2(scaled_z, scaled_y)
		headingRad += self.declination
	...
	def getAxes(self):
		offsetx = -12.5584 #Outdoormessung 2 
		offsety = -338.6939
		'''
		...
		if (magno_x == -4096):
			...
			magno_x = round((magno_x - offsetx)* self.scale, 4)
			
		if (magno_y == -4096):
			magno_y = None
		else:
			magno_y = round((magno_y -offsety)* self.scale, 4)
